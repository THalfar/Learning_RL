...
env = make_vec_env(lambda: gym.make("Ant-v4"), n_envs=6)
...
clip.write_videofile(f'/workspace/videos/ant_trial_baseline_{total_reward:.0f}.mp4', codec='libx264')
del model
del env
del eval_env
del obs
...
def objective(trial):
    ...
    clip = mpy.ImageSequenceClip(frames, fps=30)
    clip.write_videofile(f'/workspace/videos/ant_trial_{trial.number}_{total_reward:.0f}.mp4', codec='libx264')
